...
def agg_features(df):
    ...
    ----------
    df : pandas.DataFrame
        Long-form transaction table with at least the following columns:
        'acct', 'txn_day', 'amt_in', 'amt_out', 'txn_amt',
        'counterparty', 'is_self_txn_num', 'is_our_bank',
        'is_night', and 'is_peak'.
    ...
    pandas.DataFrame
        ...
        'tx_cnt', 'in_cnt', 'out_cnt', 'amt_in_sum', 'amt_out_sum',
        'amt_abs_mean', 'amt_abs_std', 'amt_max', 'active_days',
        'uniq_ctp', 'self_ratio', 'ourbank_ratio',
        'night_ratio', 'peak_ratio', 'net_flow', 'in_out_ratio'.
    """
...
def wide_count(df, col, prefix):
    ...
    For each account and each category value in `col`, this function counts
    the number of transactions and then pivots to wide form. It also computes
    per-account fractions of counts for each category.
    ...
    -------
    pandas.DataFrame
        Wide-form table with one row per account. Contains count columns
        named like ``f"{prefix}_{value}"`` and fraction columns named like
        ``f"{prefix}_pct_{value}"``.
    """
...
def counterparty_profile(df):
    ...
    This function is intended to be applied group-wise (per account). It
    computes the empirical distribution of the 'counterparty' column and
    then derives the Shannon entropy and Herfindahl index.

    Parameters
    ----------
    df : pandas.DataFrame
        Subset of the long-form transaction table for a single account,
        with a 'counterparty' column.

    Returns
    -------
    pandas.Series
        A series with two scalar features:
        - 'ctp_entropy': Shannon entropy of the counterparty distribution.
        - 'ctp_herfindahl': Herfindahl-Hirschman concentration index.
    """
...
def timebin_profile(df):
    ...
    This function is intended for per-account grouping. It counts how many
    distinct 5-minute bins are active, the dominance of the most popular
    bin, and the entropy across bins.

    Parameters
    ----------
    df : pandas.DataFrame
        Subset of the long-form transaction table for a single account,
        with a 'min5_bin' column storing time-bin identifiers.

    Returns
    -------
    pandas.Series
        A series with three features:
        - 'bin5_cnt': number of distinct active 5-minute bins.
        - 'bin5_top1_ratio': fraction of transactions in the most frequent bin.
        - 'bin5_entropy': Shannon entropy across bins.
    """
...
def bucket_profile(df, col):
    ...
    This is a helper to summarize how concentrated a given categorical
    feature is for a single account, e.g. 'channel_type' or 'currency_bucket'.
    ...
    df : pandas.DataFrame
        Subset of the long-form transaction table for a single account.
    col : str
        Column name of the categorical feature to summarize.

    Returns
    -------
    pandas.Series
        A series with two features:
        - f\"{col}_entropy\": Shannon entropy of the category distribution.
        - f\"{col}_top1\": highest category probability (top share).
    """
...
def build_activity_table(tx_long: pd.DataFrame) -> pd.DataFrame:
    ...
    This function aggregates per-account transaction counts, unique
    counterparties, and the standard deviation of absolute transaction
    amounts. It also computes the entropy of counts over 5-minute time bins.

    Parameters
    ----------
    tx_long : pandas.DataFrame
        Long-form transaction table with 'acct', 'txn_day', 'counterparty',
        'txn_amt', and 'min5_bin' columns.

    Returns
    -------
    pandas.DataFrame
        Table with one row per account and activity features:
        'tx_cnt', 'uniq_ctp', 'amt_abs_std', and 'bin5_entropy'.
    """
    if len(tx_long) == 0:
...
def normalize_numeric_features(X_train: pd.DataFrame, X_pred: pd.DataFrame):
    ...
    This function:
    1. Computes global caps on selected numeric columns from `X_train`.
    2. Applies the same caps to both `X_train` and `X_pred`.
    3. Applies log1p transform to count/amount-like columns and
       signed-log transform to net-flow columns.

    Parameters
    ----------
    X_train : pandas.DataFrame
        Training feature table, including label-related columns.
    X_pred : pandas.DataFrame
        Prediction feature table, aligned in columns with `X_train`
        except for label columns.
    ...
    tuple
        ...
        - caps : dict
            Mapping from column name to (lower_cap, upper_cap).
        - log_cols : list of str
            Columns to which log1p was applied.
        - signed_log_cols : list of str
            Columns to which signed_log1p was applied.
    """
    win_cols = [c for c in [
...
def build_train_keys(tx, alert, tx_long):
    ...
    This function:
    1. Builds positive keys from the alert table (one positive per account).
    2. Derives the last transaction day per account from the transaction table.
    3. Constructs a pool of candidate unlabeled accounts (no alert).
    4. Samples hard negatives based on activity features from `tx_long`.
    5. Merges positives and negatives into a final training key table.

    Parameters
    ----------
    tx : pandas.DataFrame
        Raw transaction table with 'from_acct', 'to_acct', and 'txn_day'.
    alert : pandas.DataFrame
        Alert table with 'acct' and 'event_day' (or columns convertible to it).
    tx_long : pandas.DataFrame
        Long-form transaction table used for activity-based negative sampling.
    ...
    tuple
        ...
        - keys : pandas.DataFrame
            Training keys with columns 'acct', 'event_day', 'label',
            and 'is_unlabeled'. Each account appears at most once.
        - acc_last : pandas.DataFrame
            Per-account table with 'acct' and 'last_txn_day'.
    """
    # 正類
...
def main():
    ...
    This function:
    1. Parses CLI arguments for input file paths and inference mode.
    2. Reads and normalizes raw transaction and alert data.
    3. Builds training keys (with labels and hard negatives) when not in
       inference-only mode.
    4. Defines time windows for training and prediction transactions.
    5. Aggregates features, aligns columns, normalizes numeric features.
    6. Writes out training and prediction feature files and a metadata JSON.

    When ``--inference_only`` is provided, only prediction features and a
    minimal metadata file are generated, without reading alerts or emitting
    training features.
    """
